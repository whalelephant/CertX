...
import (
	...
	"github.com/whalelephant/certX/MoM/testutil/network"
	"github.com/whalelephant/certX/MoM/x/employments/client/cli"
...
func networkWithRecordObjects(t *testing.T, n int) (*network.Network, []*types.Record) {
	...
	for i := 0; i < n; i++ {
		state.RecordList = append(state.RecordList, &types.Record{Creator: "ANY", Id: uint64(i)})
	}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.err != nil {
				...
				require.Equal(t, tc.obj, resp.Record)
			}
		})
	}
}
...
func TestListRecord(t *testing.T) {
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			for j := i; j < len(objs) && j < i+step; j++ {
				assert.Equal(t, objs[j], resp.Record[j-i])
			}
		}
	})
	...
	t.Run("Total", func(t *testing.T) {
		...
		require.Equal(t, objs, resp.Record)
	})
}
