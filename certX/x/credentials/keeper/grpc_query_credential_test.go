...
func TestCredentialQuerySingle(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			desc:     "First",
			request:  &types.QueryGetCredentialRequest{Id: msgs[0].Id},
			response: &types.QueryGetCredentialResponse{Credential: &msgs[0]},
		},
		{
			desc:     "Second",
			request:  &types.QueryGetCredentialRequest{Id: msgs[1].Id},
			response: &types.QueryGetCredentialResponse{Credential: &msgs[1]},
		},
		{
			desc:    "KeyNotFound",
			request: &types.QueryGetCredentialRequest{Id: uint64(len(msgs))},
		...
		t.Run(tc.desc, func(t *testing.T) {
			response, err := keeper.Credential(wctx, tc.request)
			if tc.err != nil {
...
func TestCredentialQueryPaginated(t *testing.T) {
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(msgs); i += step {
			resp, err := keeper.CredentialAll(wctx, request(nil, uint64(i), uint64(step), false))
			require.NoError(t, err)
			for j := i; j < len(msgs) && j < i+step; j++ {
				assert.Equal(t, &msgs[j], resp.Credential[j-i])
			}
		}
	})
	t.Run("ByKey", func(t *testing.T) {
		...
		var next []byte
		for i := 0; i < len(msgs); i += step {
			resp, err := keeper.CredentialAll(wctx, request(next, 0, uint64(step), false))
			require.NoError(t, err)
			for j := i; j < len(msgs) && j < i+step; j++ {
				assert.Equal(t, &msgs[j], resp.Credential[j-i])
			}
	...
	t.Run("Total", func(t *testing.T) {
		resp, err := keeper.CredentialAll(wctx, request(nil, 0, 0, true))
		require.NoError(t, err)
		require.Equal(t, len(msgs), int(resp.Pagination.Total))
	})
	t.Run("InvalidRequest", func(t *testing.T) {
		_, err := keeper.CredentialAll(wctx, nil)
