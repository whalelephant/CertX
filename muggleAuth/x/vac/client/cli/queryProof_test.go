...
import (
	...
	"github.com/whalelephant/certX/muggleAuth/testutil/network"
	"github.com/whalelephant/certX/muggleAuth/x/vac/client/cli"
...
func networkWithProofObjects(t *testing.T, n int) (*network.Network, []*types.Proof) {
	...
	for i := 0; i < n; i++ {
		state.ProofList = append(state.ProofList, &types.Proof{Creator: "ANY", Id: uint64(i)})
	}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.err != nil {
				...
				require.Equal(t, tc.obj, resp.Proof)
			}
		})
	}
}
...
func TestListProof(t *testing.T) {
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			for j := i; j < len(objs) && j < i+step; j++ {
				assert.Equal(t, objs[j], resp.Proof[j-i])
			}
		}
	})
	...
	t.Run("Total", func(t *testing.T) {
		...
		require.Equal(t, objs, resp.Proof)
	})
}
